...
export default function HistoryInterfaceAssignedCard(props: InterfaceAssignedCardProps) {
    ...
    return (
        <div className='w-full bg-white-55 flex flex-col items-center justify-center rounded-lg mb-2 shadow-md drop-shadow-[1px_1px_2px_rgba(0,0,0,0.25)]'>
            <section className='w-full px-4 py-2 bg-white-50 flex flex-row items-center rounded-t-lg'>
                <section className='w-full flex flex-row flex-wrap items-center gap-2 md:gap-2 lg:gap-8'>
                    ...
                    <div id="statusAssignment" className='min-w-fit max-w-72 flex flex-row gap-2 items-center justify-start'>
                        <h3 className='text-lg text-blue-800 font-bold self-center'>Estatus Asignación:</h3>
                        {props.data.statusAssignment === StatusAssignment.inspected && 
                            <p className='text-green-500 font-semibold'>{StatusAssignmentTranslation.inspected}</p>
                        }
                        {props.data.statusAssignment === StatusAssignment.rediscovered && 
                            <p className='text-green-500 font-semibold'>{StatusAssignmentTranslation.rediscovered}</p>
                        }
                    </div>
                </section>
            ...
            <section className='w-full px-4 flex flex-col gap-1 md:flex-row md:gap-5'>
                <h4 className='text-sm text-gray-700 font-bold'>Asignado: {props.data.dateAssignment}</h4>
                <h4 className='text-sm text-gray-700 font-bold'>Asignado por: {props.data.assignedBy}</h4>
                <h4 className='text-sm text-gray-700 font-bold'>Asignación Realizada: {props.data.updateAt}</h4>
            </section>
