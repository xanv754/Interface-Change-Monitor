...
function verifyInput(firstInput: string, secondInput: string, thirdInput: string): boolean {
    let status = true;

    if (firstInput === "") {
        badInput('alertBlankUsername');
        status = false;
    } else goodInput('alertBlankUsername');
    if (firstInput === "") {
    ...
    if ((secondInput !== "") && (thirdInput !== "") && (secondInput !== thirdInput)) {
...
export default function ViewForgotPassword(){
    const updatePassword = async (username: string, newPassword: string) => {
        await LoginController.forgotPassword(username, newPassword);
        alert('Solicitud de cambio de contraseña enviada. Si su usuario es correcto, espere a que algún administrador acepte su solicitud para iniciar sesión');
        location.href = '/';
    }
    ...
    const forgotPasswordFormHandler = (e) => {
        ...
        const status = verifyInput(username, newPassword, repeatPassword);
        if (status) updatePassword(username, newPassword);
    }
    ...
    return(
        <main className="w-full h-full flex justify-center items-center">
            <div className="h-fit w-40 min-w-fit px-4 py-8 bg-white rounded-xl flex flex-col items-center">
                ...
                <form onSubmit={forgotPasswordFormHandler} className="w-full flex flex-col gap-2 px-4">
                    <div className="flex flex-col w-full max-md:flex-col">
                        <div className="flex flex-row w-full max-md:flex-col">
                            <label htmlFor="username" className="w-70 py-2 px-4 bg-blue-500 md:rounded-l-md text-white text-center font-lexend max-md:rounded-t-md max-md:w-full">Nombre de Usuario</label>
                            <input id="username" name="username" type="username" className="w-full outline-none text-blue-500 font-lexend p-2 bg-gray-550 max-md:rounded-bl-md max-md:w-full" placeholder="Usuario" />
                        </div>
                        <small id="alertBlankUsername" className="px-2 font-lexend font-light text-red-100 hidden my-1">Usuario Requerido</small>
                    </div>
                    <div className="flex flex-col w-full max-md:flex-col">
                        <div className="flex flex-row w-full max-md:flex-col">
                    ...
                    <div className="w-full flex flex-row justify-center items-center gap-2 mt-2 max-md:flex-col">
                        <button id="btn-cancel" type="button" className="rounded-full font-lexend font-bold text-white bg-blue-500 px-6 py-2 transition-all duration-300 ease-in-out hover:bg-red-500 max-md:w-full" onClick={() => {location.href = '/login'}}>Regresar</button>
                        <button id="btn-save" type="submit" className="rounded-full font-lexend font-bold text-white bg-blue-500 px-6 py-2 transition-all duration-300 ease-in-out hover:bg-green-500 max-md:w-full">Aceptar</button>
                    </div>
                </form>
            </div>
        </main>
