...
from utils import is_valid_interface_type
...
class InterfaceController:
    ...
    @staticmethod
    def get_by_device_type(ip: str, community: str, ifIndex:int, type: str) -> dict | None:
        try:
            if not is_valid_interface_type(type): return None
    ...
    def register(body: InterfaceRegisterBody) -> bool:
        try:
            ...
            if equipment is None: return False
            id_equipment = equipment[EquipmentSchema.ID.value]
            sysname = equipment[EquipmentSchema.SYSNAME.value]
            if sysname != body.sysname: 
                EquipmentController.update_sysname(body.ip, body.community, body.sysname)
    ...
    def update_type(id: int, type: str) -> bool:
        try:
            model = Interface(id=id)
            if not model.get_by_id(): return False
