...
def register_new_operator():
    try:
        ...
        print("[bold gold3][i]Please enter the necessary data for the correct register[/i]")
        ...
        lastname = input()
        if not lastname: raise Exception("Lastname cannot be empty")
        ...
        if password != password_confirm: raise Exception("Passwords do not match")
        ...
        profile = input()
        if not profile: raise Exception("Profile cannot be empty")
        profile = profile.upper()
        if not is_valid_profile_type(profile): raise Exception("Invalid profile")
        ...
        if status:
            Log.save(f"New operator registered: {username} ({name.capitalize() + " " + lastname.capitalize()})", __file__, Log.info, console=True)
        else:
        ...
        Log.save(f"Error registering new operator: {e}", __file__, Log.error, console=True)