...
class UpdaterDatabase:
    ...
    def __init__(self, data: list):
        if not len(data) == 16: raise Exception("Data not valid")
    ...
    def update(self) -> None:
        ...
        if not old_interface_db:
            InterfaceController.update_type(interface_db[InterfaceSchema.ID.value], InterfaceType.OLD.value)
            InterfaceController.register(self.interface)
            ...
            InterfaceController.update(
                ...
                InterfaceRegisterBody(
                    ...
                    ifPromiscuousMode=interface_db[InterfaceSchema.IFPROMISCUOUSMODE.value],
                    ifConnectorPresent=interface_db[InterfaceSchema.IFCONNECTORPRESENT.value],
                    ifLastCheck=interface_db[InterfaceSchema.IFLASTCHECK.value]
                )
            )
            InterfaceController.update_type(interface_db[InterfaceSchema.ID.value], self.interface)
    ...
    def _compare_interfaces(self, interface_db: dict) -> bool:
        ...
        if self.interface.ifName != interface_db[InterfaceSchema.IFNAME.value]: return False
        if self.interface.ifDescr != interface_db[InterfaceSchema.IFDESCR.value]: return False
        if self.interface.ifAlias != interface_db[InterfaceSchema.IFALIAS.value]: return False
        if self.interface.ifHighSpeed != interface_db[InterfaceSchema.IFHIGHSPEED.value]: return False
        if self.interface.ifOperStatus != interface_db[InterfaceSchema.IFOPERSTATUS.value]: return False        
        if self.interface.ifAdminStatus != interface_db[InterfaceSchema.IFADMINSTATUS.value]: return False
